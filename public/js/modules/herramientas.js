...
const HerramientasModule = {
  ...
  _render(lista) {
    ...
    if (!lista.length) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state">
        <i class="bi bi-tools"></i><p>No hay herramientas registradas</p>
      </div></td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = lista.map((h, i) => `
      <tr>
        ...
        <td><span class="badge-garantia">${h.stock}</span></td>
        <td>${escapeHtml(h.categoria || h.nombre_categoria || h.id_categoria)}</td>
        <td>${escapeHtml(h.estado || h.nombre_estado || h.id_estado)}</td>
        <td>${escapeHtml(h.status || h.nombre_status || h.id_status)}</td>
        <td>
  ...
  _filter() {
    ...
    this._render(AppState.herramientas.filter(h =>
      ...
      h.serie.toLowerCase().includes(search)
    ));
  },
  ...
  _openModal(mode, herramienta = null) {
    ...
    document.getElementById('hCategoria').value = isEdit ? herramienta.id_categoria : '';
    document.getElementById('hEstado').value = isEdit ? herramienta.id_estado : '';
    document.getElementById('hStatus').value = isEdit ? herramienta.id_status : '';

    clearErrors(['hNombre', 'hSerie', 'hCategoria', 'hEstado', 'hStatus']);
